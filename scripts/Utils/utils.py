    ...
    def add_features_to_image(self, frame, batch):
        ...
        for i in range(len(batch)):
            ...
            if people:
                ...
                props.append(prop)
                cv2.putText(frame, str(prop)+'%' + ' - ' + str(c),
                            pi, font, 1, (255, 150, 0), 2, cv2.LINE_AA)
                if 'name' in people:
                    cv2.rectangle(frame, (pi[0], pf[1]+50),
                                  (pf), (0, 0, 255), cv2.FILLED)
                    cv2.putText(frame, str(
                        people['name']), (pi[0]+10, pf[1]+30), font, 1, (255, 150, 0), 2, cv2.LINE_AA)
                else:
                    cv2.putText(
                        frame, "Unknow", (pi[0]+10, pf[1]+30), font, 1, (255, 150, 0), 2, cv2.LINE_AA)
        ...
        if len(props):
            if max(props) >= self.percent_of_face:
                ...
                cv2.rectangle(frame, cords[index][0],
                              cords[index][1], (0, 0, 255), 5)
